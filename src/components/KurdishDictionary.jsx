...
// ==================== ستایلە هاوبەشەکان بۆ هەڵبژاردنی تیمی لای ڕاست و مۆدالی کلیل ====================
const SIDEBAR_THEMES_CSS = `
  ...
  .theme-btn.key-btn { background: #1e1e2f !important; color: gold !important; }
  ...
  @media (max-width: 640px) {
    .theme-sidebar { top: auto; bottom: 20px; right: 20px; transform: none; flex-direction: row; border-radius: 60px; padding: 8px 12px; gap: 8px; }
  ...
  .key-modal {
    background: white; padding: 1.5rem; border-radius: 24px; width: 280px;
    text-align: center; direction: rtl; font-family: system-ui;
  }
  .key-modal input {
    width: 100%; padding: 8px; margin: 12px 0; border: 1px solid #ccc;
    border-radius: 40px; direction: ltr; text-align: left;
  }
  .key-buttons button {
    margin: 4px; padding: 6px 12px; border: none; border-radius: 30px;
    background: #2c7da0; color: white; cursor: pointer;
  }
`;
...
export default function KurdishDictionaryAllThemes() {
  ...
  const fetchExamples = async (word, isKu) => {
    if (groqKey) {
      ...
      throw new Error("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");
    }
  };
  ...
  const doSearch = async (word) => {
    ...
    try {
      ...
      try {
        ...
        setExamples([]);
        if (!groqKey) setError("بۆ دیارکردنی نمونەی ڕستە، پێویستە Groq API Key دابنێ. کلیک لە 🔑 بکە.")
      } finally {
        setExLoading(false);
      }
    } catch {
  ...
  return (
    <>
      ...
      {/* ───── ستونی هەڵبژاردنی ڕووکار + دوگمەی کیلی Groq ───── */}
        ...
        <div className="key-modal-backdrop" onClick={() => setShowKeyInput(false)}>
          <div className="key-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="key-buttons">
              <button onClick={() => saveGroqKey(tempKeyRef.current.value)}>پاشەکەوت</button>
              <button onClick={() => setShowKeyInput(false)}>داخستن</button>
            </div>
            <small>Keyەکە تەنها لە ناو browserـی تۆدا هەڵدەگیرێت.</small>
          </div>
        </div>
      )}
          ...
          <div className="ck-page">
            ...
            <div className="ck-search">
              <div className="ck-input-row">
                <input ref={inputRef} className="ck-input" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && doSearch(query)} placeholder="وشە بنووسە · Type a word…" />
                <button className="ck-btn" onClick={() => doSearch(query)} disabled={loading || !query.trim()}>{loading ? <span className="ck-spinning">⊙</span> : "←"}</button>
              </div>
            </div>
            ...
            {result && !loading && (
              <div className="ck-card">
                ...
                {exLoading && <div className="ck-loading"><span className="ck-spinning">⊙</span> نمونەکان دەگەیەنرێن…</div>}
                {!exLoading && examples.length > 0 && (
                  <div className="ck-examples">{examples.map((ex, i) => (
                    <div key={i} className="ck-example">
                      <div className="ck-ex-num">{i+1}.</div>
                      <div><div className="ck-ex-ku">{isKu ? ex.ku : ex.en}</div><div className="ck-ex-en">{isKu ? ex.en : ex.ku}</div></div>
                    </div>
                  ))}</div>
                )}
              </div>
            )}
            {history.length > 1 && <div className="ck-chips-section"><div className="ck-chips-head">مێژووی گەڕان · Recent</div><div className="ck-chips">{history.slice(1).map((w,i) => <button key={i} className="ck-chip" onClick={() => doSearch(w)}>{w}</button>)}</div></div>}
          ...
          <div className="md-page">
            ...
            <div className="md-search">
              <div className="md-input-row">
                <input ref={inputRef} className="md-input" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && doSearch(query)} placeholder="وشە بنووسە · Type a word…" />
                <button className="md-btn" onClick={() => doSearch(query)} disabled={loading || !query.trim()}>{loading ? <span className="md-spinning">⊙</span> : "→"}</button>
              </div>
            </div>
              ...
              <div className="md-card">
                ...
                <div className="md-card-body">
                  ...
                  <div className="md-section">
                    ...
                    {exLoading && <div className="md-loading"><span className="md-spinning">⊙</span> نمونەکان دەگەیەنرێن…</div>}
                    {!exLoading && examples.length > 0 && (
                      <div className="md-examples">{examples.map((ex,i) => (
                        <div key={i} className="md-example">
                          <div className="md-ex-num">0{i+1}</div>
                          <div><div className="md-ex-ku">{isKu ? ex.ku : ex.en}</div><div className="md-ex-en">{isKu ? ex.en : ex.ku}</div></div>
                        </div>
                      ))}</div>
                    )}
                  </div>
            ...
            {history.length > 1 && <div className="md-chips-section"><div className="md-chips-head">Recent Searches</div><div className="md-chips">{history.slice(1).map((w,i) => <button key={i} className="md-chip" onClick={() => doSearch(w)}>{w}</button>)}</div></div>}
          ...
          <div className="gd-page">
            ...
            <div className="gd-search">
              <div className="gd-input-row">
                <input ref={inputRef} className="gd-input" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && doSearch(query)} placeholder="وشە بنووسە · Type a word…" />
                <button className="gd-btn" onClick={() => doSearch(query)} disabled={loading || !query.trim()}>{loading ? <span className="gd-spinning">⊙</span> : "←"}</button>
              </div>
            </div>
            ...
            {result && !loading && (
              <div className="gd-card">
                <div className="gd-card-head"><div className="gd-word">{result.word}</div><div className="gd-dir-badge">{isKu ? "KU → EN" : "EN → KU"}</div></div>
                <div className="gd-card-body">
                  ...
                  <div className="gd-section">
                    ...
                    {exLoading && <div className="gd-loading"><span className="gd-spinning">⊙</span> نمونەکان دەگەیەنرێن…</div>}
                    {!exLoading && examples.length > 0 && (
                      <div className="gd-examples">{examples.map((ex,i) => (
                        <div key={i} className="gd-example">
                          <div className="gd-ex-num">{i+1}</div>
                          <div><div className="gd-ex-ku">{isKu ? ex.ku : ex.en}</div><div className="gd-ex-en">{isKu ? ex.en : ex.ku}</div></div>
                        </div>
                      ))}</div>
                    )}
                  </div>
            ...
            {history.length > 1 && <div className="gd-chips-section"><div className="gd-chips-head">مێژووی گەڕان · Recent</div><div className="gd-chips">{history.slice(1).map((w,i) => <button key={i} className="gd-chip" onClick={() => doSearch(w)}>{w}</button>)}</div></div>}
        ...
        <div className="gl-root">
          <div className="gl-page">
            ...
            <div className="gl-search">
              <input ref={inputRef} className="gl-input" value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && doSearch(query)} placeholder="وشە بنووسە · Type a word…" />
              <button className="gl-btn" onClick={() => doSearch(query)} disabled={loading || !query.trim()}>{loading ? <span className="gl-spinning">⊙</span> : "→"}</button>
            </div>
              ...
              <div className="gl-card">
                ...
                <div className="gl-card-body">
                  ...
                  <div className="gl-section">
                    ...
                    {exLoading && <div className="gl-loading"><span className="gl-spinning">⊙</span><span>نمونەکان دەگەیەنرێن…</span></div>}
                    {!exLoading && examples.length > 0 && (
                      <div className="gl-examples">{examples.map((ex,i) => (
                        <div key={i} className="gl-example">
                          <div className="gl-ex-ku">{isKu ? ex.ku : ex.ku}</div>
                          <div className="gl-ex-en">{isKu ? ex.en : ex.en}</div>
                        </div>
                      ))}</div>
                    )}
                  </div>
            ...
            {history.length > 1 && <div className="gl-chips-section"><div className="gl-chips-label">مێژووی گەڕان · Recent</div><div className="gl-chips">{history.slice(1).map((w,i) => <button key={i} className="gl-chip" onClick={() => doSearch(w)}>{w}</button>)}</div></div>}
